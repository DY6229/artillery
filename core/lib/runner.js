...
function runner(script, payload, options, callback) {
  ...
  if (script.config.customFunctions) {
    ...
    script.config.customFunctions = require(customFunctionsPath);
  }
  ...
  return handleBeforeRequests(script, runnableScript, runnerEngines, ee)
    .then(function() {
      //
      // load plugins:
      //
...
function createContext(script) {
  ...
  let customFunctions = script.config.customFunctions;
